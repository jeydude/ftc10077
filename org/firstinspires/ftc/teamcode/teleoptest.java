...
public class week3teleop extends LinearOpMode {
    ...
    private static int BALL_KICK_TIME = 600;
    private static int KICKER_WAIT_TIME = 400;
    private static int BALL_FEED_TIME = 500;
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (gamepad1.y && state == AutoState.DONE) {
    ...
    void runAutoShoot() {
        ...
        switch (state) {
            case START:
                topKicker.setPosition(topKickerDownPosition);
                shooterOn(shooterPower);
                if (stateTimer.milliseconds() >=  KICKER_WAIT_TIME) {
                    stateTimer.reset();
            ...
            case BALL1_FEED:
                beltOn(0.9);
                if (stateTimer.milliseconds() >= BALL_FEED_TIME) {
                    beltOff();
                    stateTimer.reset();
     ...
     */
    private void runKicker(int ballnumber) {
