 ...
 */
@Autonomous(name = "AutonomousVelocity2", group = "Drive")
public class AutonomousVelocity2 extends LinearOpMode {
    ...
    // Motor constants for shooter
    private static final double SHOOTER_TICKS_PER_REV = 28;  // GripForce 312 RPM
    private static final double MOTOR_RPM = 312;     // motor max RPM
    ...
    public void runOpMode() {
        ...
        if (opModeIsActive()) {
            ...
            double targetRPM = 200; // tune for field distance
           // Determine battery-compensated shooter power
            double shooterPower = calculateShooterPower();
            double baseRPM = 200; // tuned RPM for field
            double adjustedRPM = baseRPM * shooterPower;

            maintainShooterRPM(adjustedRPM); // keep shooter at target RPM
            sleep(50); // short wait to stabilize

            // -------- BALL 1 --------
            kickBall();
            beltRampDown(150); // smooth stop
            sleep(100);       // buffer before next ball
            ...
            // Gradually lift ball
            beltRampUp(0.5, 200);
            sleep(50); // ensure ball is under kicker
            // -------- BALL 2 --------
            kickBall();
            beltRampDown(150); // smooth stop
            sleep(100);       // buffer before next ball
    ...
    */
    ...
    // --- Belt with smooth ramping ---
    private void beltRampUp(double maxPower, int rampTimeMs) {
        int steps = 5;
        for (int i = 1; i <= steps; i++) {
            belt.setPower(maxPower * i / steps);
            sleep(rampTimeMs / steps);
        }
    }
    // Turns on the belt at specified power
    private void beltRampDown(int rampTimeMs) {
        int steps = 5;
        double currentPower = belt.getPower();
        for (int i = steps; i >= 0; i--) {
            belt.setPower(currentPower * i / steps);
            sleep(rampTimeMs / steps);
        }
        beltOff();
    }
    ...
    */
    private void shooterVelocityOn(double targetRPM) {
        double ticksPerSec = rpmToTicksPerSecond(targetRPM);

        shooterLeft.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        shooterRight.setMode(DcMotor.RunMode.RUN_USING_ENCODER);

        shooterRight.setDirection(DcMotor.Direction.REVERSE);  // reverse one side
    ...
    // Maintain target RPM using proportional feedback
    private void maintainShooterRPM(double targetRPM) {
        double Kp = 0.001; // tune as needed
        ...
        shooterLeft.setVelocity(targetTicks);
        shooterRight.setVelocity(targetTicks);

        // Short feedback loop
        for (int i = 0; i < 10; i++) {
            double leftTicks = shooterLeft.getVelocity();
            double rightTicks = shooterRight.getVelocity();

            double errorLeft  = targetTicks - leftTicks;
            double errorRight = targetTicks - rightTicks;

            shooterLeft.setVelocity(leftTicks + Kp * errorLeft);
            shooterRight.setVelocity(rightTicks + Kp * errorRight);

            sleep(50); // small delay between adjustments
        }
    }

    // Optional: telemetry for shooter RPM
    private void showShooterRPM() {
        double leftRPM = shooterLeft.getVelocity() * 60.0 / TICKS_PER_REV;
        double rightRPM = shooterRight.getVelocity() * 60.0 / TICKS_PER_REV;
        telemetry.addData("Shooter Left RPM", leftRPM);
        telemetry.addData("Shooter Right RPM", rightRPM);
        telemetry.addData("Belt Power", belt.getPower());
        telemetry.update();
    }
